...
for file in "$input_dir"/*R1.trim.fastq.gz; do
  ...
  ime=$(basename "$file" _R1.trim.fastq.gz)
  srun --ntasks=1 --cpus-per-task=10 bwa mem "$ref" "$file" ${file%R1.trim.fastq.gz}R2.trim.fastq.gz > $input_dir/${ime}.sam &
done
wait
